...
export const Sidenav = () => {
  ...
  const mobileItems = [
    ...
    { id: "location", label: "POINTS", icon: <MapPin className="w-5 h-5" /> },
    { id: "analytics", label: "DATA", icon: <TrendingUp className="w-5 h-5" /> },
  ...
  return (
    <>
      ...
      <aside className="hidden md:flex flex-col items-center justify-between w-16 h-screen py-6 bg-linear-to-b from-[#222f3f] to-[#01060e] border-r border-[#1E293B]/30 select-none z-50">
        ...
        <div 
          ...
          className={`group relative flex items-center justify-center w-12 h-12 transition-all duration-300 cursor-pointer ${
            activeTab === "profile" ? "text-white scale-105" : "text-[#4F5B73] hover:text-slate-200"
          }`}
        >
      ...
      <nav className="flex md:hidden fixed bottom-0 left-0 right-0 h-16 bg-[#090D1A]/95 backdrop-blur-md border-t border-slate-800/40 items-center justify-around px-4 select-none z-50 shadow-2xl">
          ...
          return (
            <button
              ...
              className={`flex flex-col items-center justify-center gap-1 cursor-pointer transition-all duration-300 ${
                isActive ? "text-sky-400 scale-105" : "text-[#4F5B73] hover:text-slate-200"
              }`}
            >
              <div className="transition-transform duration-300">
                {item.icon}
              </div>
              <span className="text-[8px] font-black tracking-widest leading-none mt-0.5">{item.label}</span>
            </button>
          );
        })}
      </nav>
